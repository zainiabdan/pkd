        ...
        <div class="form-group">
         ...
         {{-- <div class="form-group">
            <select name="sopir" class="form-control">
                <option  @php if(false) { echo 'selected="selected"'; } @endphp value="" >Pilih Sopir</option>
                @foreach ($drivers as $item)
                    <option  @php if(false) { echo 'selected="selected"'; } @endphp value="{{ $item->id_driver }}">{{  $item->name  }}</option>
                @endforeach
            </select>
        </div> --}}
